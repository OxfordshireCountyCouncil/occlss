...
describe('Homepage', () => {
  ...
  async function setup (page) {
    $navigation = await page.$('.js-doc-navigation')
    $navigationToggler = await page.$('.js-doc-navigation__toggler')
  }
    ...
    describe('when menu button is pressed', () => {
      it('should apply the corresponding open state class to the menu button', async () => {
        await expect(getAttribute($navigationToggler, 'class')).resolves
          .not.toContain('doc-header__menu-button--open')

        await $navigationToggler.click()

        // Menu button open state
        await expect(getAttribute($navigationToggler, 'class')).resolves
          .toContain('doc-header__menu-button--open')
      })
      ...
      it('should indicate the open state of the navigation', async () => {
        await expect(getAttribute($navigation, 'class')).resolves.not.toContain('doc-navigation--active')

        await $navigationToggler.click()

        // Menu open state
        await expect(getAttribute($navigation, 'class')).resolves.toContain('doc-navigation--active')
      })
