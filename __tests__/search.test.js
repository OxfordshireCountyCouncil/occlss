...
describe('Site search', () => {
  ...
  async function setup (page) {
    ...
    $wrapper = await $module.$('.doc-site-search__wrapper')
    $searchInput = await $module.$('.doc-site-search__input')
  }
  ...
  it('does not return any results when searching for something that does not exist', async () => {
    ...
    const $searchOptions = await $module.$$('.doc-site-search__option')
  ...
  it('returns results where a word in the title begins with the letter "d"', async () => {
    ...
    const resultsArray = await page.evaluate(
      () => [...document.querySelectorAll('.doc-site-search__option')]
        // ignore any results where a match was found in the alias
        .filter(elem => !elem.querySelector('.doc-site-search__aliases'))
  ...
  it('returns results that contain aliases that start with the letter "d"', async () => {
    ...
    const resultsArray = await page.evaluate(
      () => [...document.querySelectorAll('.doc-site-search__option')]
        ...
        .filter(elem => elem.querySelector('.doc-site-search__aliases'))
        .map(elem => elem.querySelector('.doc-site-search__aliases').textContent)
    )
  ...
  it('doesn\'t show any aliases if it finds any matches in the title', async () => {
    ...
    const resultsArray = await page.evaluate(
      () => [...document.querySelectorAll('.doc-site-search__aliases')]
        .map(elem => elem.querySelector('.doc-site-search__aliases'))
    )
  ...
  it('should focus the input when clicking the button', async () => {
    ...
    const $searchInput = await page.evaluate(() => document.querySelector('.doc-site-search__input'))
