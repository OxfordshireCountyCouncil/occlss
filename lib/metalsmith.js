...
// extract page headings
  ...
  .use(sass({
    quietDeps: true,
    loadPaths: ['node_modules', 'src/stylesheets', 'src/occlss-scss', 'src/sass-new','node_modules/reflex-grid/scss','node_modules/breakpoint-sass/stylesheets'], // an array of paths that sass can look in when attempt to resolve @import declarations
  ...
  .use((files, metalsmith, done) => {
    ...
    for (const [filename, file] of Object.entries(files)) {
      ...
      } else if (filename.startsWith('sass-new/')) {
        if (filename.endsWith('.css') || filename.endsWith('.css.map')) {
          // Move CSS and CSS map files from sass-new to assets/css
          const newFilename = filename.replace('sass-new/', 'assets/css/')
          newFiles[newFilename] = file
        }
        // Skip other files from sass-new (like _rem.scss, _headings.scss)
      } else {
  ...
  .use(async (files, metalsmith, done) => {
    ...
    const componentStylesheets = componentCssAssets.map(asset => path.join('components', asset))
    ...
    const customStylesheets = customCssAssets.map(asset => path.join('assets/css', asset))
