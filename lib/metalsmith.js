...
const getFW = require('./fw-version.js') // apply framework version to the footer
...
const nunjucksOptions = {
  ...
  globals: {
    ...
    getClassesFromArray: fileHelper.getClassesFromArray
  },
...
// extract page headings
  ...
  .use(sass({
    quietDeps: true,
    loadPaths: ['node_modules', 'src/stylesheets', 'src/occlss-scss', 'node_modules/reflex-grid/scss'], // an array of paths that sass can look in when attempt to resolve @import declarations
    sourceMapIncludeSources: true,
    sourceMap: true
  }))

  // Move compiled SASS files from occlss-scss and sass-new to assets/css
  .use((files, metalsmith, done) => {
    const newFiles = {}
    
    for (const [filename, file] of Object.entries(files)) {
      if (filename.startsWith('occlss-scss/')) {
        if (filename.endsWith('.css') || filename.endsWith('.css.map')) {
          // Move CSS and CSS map files from occlss-scss to assets/css
          const newFilename = filename.replace('occlss-scss/', 'assets/css/')
          newFiles[newFilename] = file
        }
        // Skip other files from occlss-scss (like tools folder)
      } else {
  ...
  .use(async (files, metalsmith, done) => {
    ...
    await Promise.all([
      ...
      // Copy component-level CSS from views/components into the build so they can be fingerprinted
      copyAssets('**/*.css', {
        cwd: paths.comp,
        dest: 'components'
      }),
      // Copy custom CSS files from src/assets/css into the build so they can be fingerprinted
      copyAssets('**/*.css', {
        cwd: 'src/assets/css',
        dest: 'assets/css'
      })
    ...
    // Expose a list of component CSS files to templates for dynamic inclusion
    const componentCssAssets = await glob('**/*.css', { cwd: paths.comp })
    
    // Sort component CSS assets alphabetically
    const sortedComponentCssAssets = componentCssAssets.sort((a, b) => a.localeCompare(b))
    
    const componentStylesheets = sortedComponentCssAssets.map(asset => path.join('components', asset))
    
    // Expose a list of custom CSS files from src/assets/css for dynamic inclusion
    const customCssAssets = await glob('**/*.css', { cwd: 'src/assets/css' })
    
    // Sort CSS assets by folder numeric prefixes and then by filename numeric prefixes
    const sortedCustomCssAssets = customCssAssets.sort((a, b) => {
      const aParts = a.split(path.sep)
      const bParts = b.split(path.sep)
      
      // Folder level (first segment)
      const aFolder = aParts[0] || ''
      const bFolder = bParts[0] || ''
      
      // Support both space and hyphen after number (e.g., "1 generic" or "1-generic")
      const parseWithNumericPrefix = (value) => {
        const match = (value || '').match(/^(\d+)[\s-]*(.*)/)
        if (match) {
          return { hasNum: true, num: parseInt(match[1], 10), rest: match[2] || '' }
        }
        return { hasNum: false, num: 0, rest: value || '' }
      }
      
      const aFolderInfo = parseWithNumericPrefix(aFolder)
      const bFolderInfo = parseWithNumericPrefix(bFolder)
      
      // Compare folders by numeric prefix first
      if (aFolderInfo.hasNum && bFolderInfo.hasNum) {
        if (aFolderInfo.num !== bFolderInfo.num) {
          return aFolderInfo.num - bFolderInfo.num
        }
        const folderRestCompare = aFolderInfo.rest.localeCompare(bFolderInfo.rest)
        if (folderRestCompare !== 0) return folderRestCompare
      } else if (aFolderInfo.hasNum) {
        return -1
      } else if (bFolderInfo.hasNum) {
        return 1
      } else {
        const folderAlphaCompare = aFolder.localeCompare(bFolder)
        if (folderAlphaCompare !== 0) return folderAlphaCompare
      }
      
      // If folders are equivalent, compare filenames within the same folder
      const aFile = aParts[aParts.length - 1] || ''
      const bFile = bParts[bParts.length - 1] || ''
      const aFileInfo = parseWithNumericPrefix(aFile)
      const bFileInfo = parseWithNumericPrefix(bFile)
      
      if (aFileInfo.hasNum && bFileInfo.hasNum) {
        if (aFileInfo.num !== bFileInfo.num) {
          return aFileInfo.num - bFileInfo.num
        }
        return aFileInfo.rest.localeCompare(bFileInfo.rest)
      } else if (aFileInfo.hasNum) {
        // Files with numeric prefixes (e.g., 1-...) come before others
        return -1
      } else if (bFileInfo.hasNum) {
        return 1
      }
      
      // Fallback to full path alphabetical
      return a.localeCompare(b)
    })
    
    const customStylesheets = sortedCustomCssAssets.map(asset => path.join('assets/css', asset))
    
    const siteMetadata = metalsmith.metadata()
    siteMetadata.componentStylesheets = componentStylesheets
    siteMetadata.customStylesheets = customStylesheets
  ...
  .use(canonical({
    hostname: 'https://occlss.oxfordshire.gov.uk',
    omitIndex: true,
