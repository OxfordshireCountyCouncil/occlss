...
// extract page headings
  ...
  .use(async (files, metalsmith, done) => {
    ...
    // Sort CSS assets by folder names with numeric prefixes (e.g., "1 generic", "2 base")
    const sortedCustomCssAssets = customCssAssets.sort((a, b) => {
      const aPath = a.split(path.sep)
      const bPath = b.split(path.sep)
      
      // Get the first folder name (which contains the numeric prefix)
      const aFirstFolder = aPath[0] || ''
      const bFirstFolder = bPath[0] || ''
      
      // Extract numeric prefix from folder names
      const aMatch = aFirstFolder.match(/^(\d+)\s*(.*)/)
      const bMatch = bFirstFolder.match(/^(\d+)\s*(.*)/)
      
      if (aMatch && bMatch) {
        // Both have numeric prefixes, sort by number first, then alphabetically
        ...
        // If numbers are the same, sort alphabetically by the rest of the folder name
        ...
        return aFirstFolder.localeCompare(bFirstFolder)
      }
    })
