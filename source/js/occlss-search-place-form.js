(function (document, window, $, undefined) {
    ...
    $("#js-occlss-search-place-form-postcode").submit(function (event) {
        var url = $(this).find('.occlss-form-cntrls__input').val();
        if (url) {
    ...
    $("#js-occlss-search-place-form-link").on("click", function () {
        ...
        function success(position) {
            point = new Object();
            point.x = position.coords.latitude;
            point.y = position.coords.longitude;
            osref = gridrefLetToNum(LatLongToOSGrid(point));
            window.location.href = 'https://www.oxfordshire.gov.uk/cms/near-me/' + osref[0] + '/' + osref[1] + '/GEO/Your approximate location'
        }
        function error() {
    ...
    */
    function LatLongToOSGrid(p) {
        var lat = p.x.toRad(), lon = p.y.toRad();
